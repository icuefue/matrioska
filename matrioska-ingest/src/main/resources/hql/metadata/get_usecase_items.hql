...
from {mtrsk}.mtrsk_src_rdbms_tables items_table
join {mtrsk}.mtrsk_src_usecases usecases on usecases.name = "$1" and usecases.cod_usecase = items_table.cod_usecase
left join {mtrsk}.mtrsk_src_partition_types partition_types on items_table.cod_partition_type = partition_types.cod_partition
left join {mtrsk}.mtrsk_src_delimiters delimiters_field on items_table.cod_field_delimiter = delimiters_field.cod_delimiter
left join {mtrsk}.mtrsk_src_delimiters delimiters_line on items_table.cod_line_delimiter = delimiters_line.cod_delimiter